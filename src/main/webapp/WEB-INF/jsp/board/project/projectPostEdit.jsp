...
<%@ taglib uri="http://www.springframework.org/security/tags" prefix="security" %>    
    ...
    <input type="hidden" name="boardId" value="${projectBoard.boardId }">
    <input type="hidden" name="postId" value="${projectBoard.postId }">
...
<%-- 						                	<a href="${cPath }/board/project/projectBoardView.do?what=${project.projectId }" class="btn btn-primary w-24 inline-block mr-1 mb-2">확인</a> --%>
                    ...
                    <button type="button" class="btn btn-primary-soft flex w-24 inline-block mr-1 mb-2" onclick="updateForm()"> <i icon-name="file-text" class="w-4 h-4 mr-2"></i> 저장 </button>
                </div>
            </div>
            <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
                <!-- BEGIN: Post Content -->
                <div class="intro-y col-span-12 lg:col-span-8">
                    <input type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="제목" name="postTitle" maxlength="130" value="${projectBoard.postTitle }">
                        ...
                        <div class="post__content tab-content">
                            <div id="content" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                                <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5" style="height:900px;">
                                    ...
                                    <div class="mt-5">
                                        <div >
                                            <textarea id="editor" placeholder="작성" name="postContent">${projectBoard.postContent }</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-span-12 lg:col-span-4">
                    <div class="intro-y box p-5">
                       <div class="mt-2">
			              <select class="form-select w-full" name="postAsap" id="postAsap" value="">
                             ...
                             <div class="mt-5">
                                     <label class="form-label">시작 기간</label>
                                         <div class="px-4 pb-4 flex items-center cursor-pointer relative">
                                             <input type="date" name="postSday" id="postSday" value="${projectBoard.postSday }">
                                         </div>
                                         
                                     <label class="form-label">마감 기간</label>
                                         <div class="px-4 pb-4 flex items-center cursor-pointer relative">
                                             <input type="date" name="postFday" value="${projectBoard.postSday }">
                                         </div>
                             </div>
                         </div>
                         
                         <div class="mt-7">
			              <select class="form-select w-full" name="postProgress" id="postProgress" value="${projectBoard.postProgress }">
...
// 화면(html) 브라우저에 정상적으로 출력이 된 후에 실행 하겠다.
$(document).ready(function(){
	$("#postProgress").val('${projectBoard.postProgress}').prop("selected", true); 
	$("#postAsap").val('${projectBoard.postAsap}').prop("selected", true); 
	
	$("#postSday").val('${projectBoard.postSday}');
...
function updateForm() {
    ...
    var postContent = editor.getData();
    var postSday = $("[name=postSday]").val();
    var postFday = $("[name=postFday]").val();
    var postAsap = $("[name=postAsap]").val();
    var postProgress = $("[name=postProgress]").val();
	...
	$.ajax({
		url: "${cPath}/board/project/projectPostUpdate.do",
		type: "post",
            ...
            if(data > 0){
				Swal.fire({
                    ...
                    if (result.isConfirmed) {
                    	...
                    	location.href = `${cPath }/board/project/projectPostView.do?postId=\${postId }&boardId=\${boardId}&projectId=\${projectId}`;
                    }
                })
            } else {
                Swal.fire({
