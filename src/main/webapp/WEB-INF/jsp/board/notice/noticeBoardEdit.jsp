...
<%@ taglib uri="http://www.springframework.org/security/tags" prefix="security" %>    
    ...
    <security:csrfInput/>
<%--    <input type="hidden" name="postId"  value="${noticeVO.postId}">--%>
        <div class="content" id="black_bg">
            <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
                ...
                <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
						        ...
						        <div class="modal-content">
						            <div class="modal-body p-0">
						                ...
						                <div class="px-5 pb-8 text-center">
						                	...
						                	<a href="${cPath }/board/notice/noticeBoardList.do" class="btn btn-danger w-24 inline-block mr-1 mb-2">확인</a>
						                </div>
						            </div>
						        </div>
						    </div>
                    ...
                    <!-- BEGIN: Modal Notice Update Content -->
                    <div id="update-modal-preview" class="modal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-body p-0">
                                    <input id="postId" type="hidden" value="${noticeVO.postId}">
                                    <div class="p-5 text-center"> <i data-lucide="x-circle" class="w-16 h-16 text-danger mx-auto mt-3"></i>
                                        <div class="text-3xl mt-5">정말 수정 하시겠습니까?</div>
<%--                                        <div class="text-slate-500 mt-2">아직 저장되지 않은 내용이 있습니다.</div>--%>
                                    </div>
                                    <div class="px-5 pb-8 text-center">
                                        <button data-tw-dismiss="modal" class="btn btn-secondary w-24 mr-1">취소</button>
                                        <button data-tw-dismiss="modal" class="btn btn-primary w-24 mr-1" onclick="updateNotice()">확인</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- END: Modal Notice Update Content -->
                    ...
                    <button type="button" class="btn btn-primary-soft flex w-24 inline-block mr-1 mb-2" onclick="openUpdateModal()"> <i icon-name="file-text" class="w-4 h-4 mr-2"></i> 수정 </button>
                </div>
            </div>
            <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
                <!-- BEGIN: Post Content -->
                <div class="intro-y col-span-12 lg:col-span-8">
                    <input type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="제목" name="postTitle" maxlength="130" value="${noticeVO.postTitle}">
                        ...
                        <div class="post__content tab-content">
                            <div id="content" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                                <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5" style="height:900px;">
                                    ...
                                    <div class="mt-5">
                                        <div >
                                            <textarea id="editor" placeholder="작성" name="postContent">${noticeVO.postContent }</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
function openUpdateModal() {
    var Modal = tailwind.Modal.getOrCreateInstance(document.querySelector("#update-modal-preview"));
    Modal.show();
}
...
function updateNotice() {
    ...
    $.ajax({
        url: postId,
        type: 'post',
        data: JSON.stringify({postTitle:postTitle, postContent:postContent}),
