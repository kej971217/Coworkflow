                                 ...
                                 <div class="mt-3">
                                     ...
                                     <div class="border-2 border-dashed dark:border-darkmode-400 rounded-md pt-4">
                                         ...
                                         <div class="px-4 pb-4 flex items-center cursor-pointer relative">
                                             <i icon-name="image" class="w-4 h-4 mr-2"></i> <span class="text-primary mr-1">파일 올리기</span>
                                             <input type="file" class="w-full h-full top-0 left-0 absolute opacity-0" name="addFiles" id="addFiles" multiple style="cursor:pointer;">
                                         </div>
                                     </div>
                                 </div>
                             </div>
...
// 	console.log("form",document.querySelector("#insertForm"));
	// 게시글 등록 ajax
	...
	for(let val of formData.values()){
		console.log("누느로 화긴1:",val);		
	}

	for(let val of formData.keys()){
		console.log("누느로 화긴2:",val);		
	}
	*/
	
	
	$.ajax({
		url: "${cPath}/board/notice/noticeBoardInsert.do",
		type: "post",
		data: formData,
        // 제이쿼리가 폼 데이터를 url-encoded 방식으로 디폴트 값으로 강제로 바꿈! (못 바꾸게 false 부분 두개 필수)
		contentType: false,
        processData: false,
        cache: false,
        dataType:"json",
		...
		success : function(data){
				...
				Swal.fire({
                    ...
                    text: "글을 등록했습니다.",
                                ...
                                if (result.isConfirmed) {
                                    location.href = '${cPath}/board/notice/noticeBoardList.do';
                                }
                            })
                        } else {
                            Swal.fire({
            ...
            let alarmData = {
            			// 현재 알람을 보내는 사람 ID
            			empId : "a100005",
            			alarmContent : `님 새로운 공지사항이 등록되었습니다.`,
            			almType : `NOTICE`
            		};         
...
//              console.log("이거는 타는지?")
    ...
    function oneFile(pFile){
        ...
        fileReader.onload = function(){
            ...
            chImg.src = fileReader.result;
            chImg.width = 100; chImg.height = 100;
            Disp.appendChild(chImg);
        }
