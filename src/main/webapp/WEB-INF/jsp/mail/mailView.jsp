	...
	if (serverJSON.length > 0) {
		{
			...
			var receive = mailC['sender'];
			var tANode = document.createTextNode(receive);
			from.appendChild(tANode);

			var subject = mailC['mailInboxTitle'];
			var sub = document.querySelector(".sub");
			var sb = document.createTextNode(subject);
			sub.appendChild(sb);

			var content = mailC['mailInboxContent'];
		...
		if (mailItem.hasOwnProperty("mailAttachmentId") > 0) {
			...
			eachrf.setAttribute("class", "ml-4 mr-4 mt-4 mb-10 pt-3 pb-3 pl-3 pr-3");
			eachrf.style.display = "center";
			eachrf.style.border = "1px solid grey";
			...
			for (let i = 0; i < serverJSON.length; i++) {
				...
				click.href = "${cPath}/mail/mailInbox/attachDown.do?what="
						+ mid + "&for=" + aid;
